                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {                           
                            ...
                            //进行黎曼积分求系数
                            SHCoeffiecents[k] += Le * dw * basisFunc;
                        }
                    }
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            for (int i = 0; i < mesh->getVertexCount(); i++) {
                const Point3f& v = mesh->getVertexPositions().col(i);
                const Normal3f& n = mesh->getVertexNormals().col(i);
                auto shFunc = [&](double phi, double theta) {
                    Eigen::Array3d d = sh::ToVector(phi, theta);
                    const auto wi = Vector3f(d.x(), d.y(), d.z());
                    double H = wi.dot(n);
                    Ray3f ray(v, wi.normalized());
                    ...
                    if (H > 0.0 && !scene->rayIntersect(ray, its)) {
                        // 一次反射后碰撞到的三角形
                        Vector3f v11 = mesh->getVertexPositions().col(its.tri_index.x());
                        Vector3f v12 = mesh->getVertexPositions().col(its.tri_index.y());
                        Vector3f v13 = mesh->getVertexPositions().col(its.tri_index.z());
                        Vector3f n11 = mesh->getVertexNormals().col(its.tri_index.x());
                        Vector3f n12 = mesh->getVertexNormals().col(its.tri_index.x());
                        Vector3f n13 = mesh->getVertexNormals().col(its.tri_index.x());

                        Vector3f bary = its.bary;

                        Vector3f vertex1 = v11 * bary.x() + v12 * bary.y() + v13 * bary.z();

                        // 计算交点的SH
                        auto ShFuncBounce = [&](double phi, double theta) -> double {
                            Eigen::Array3d d1 = sh::ToVector(phi, theta);
                            const auto wi1 = Vector3f(d.x(), d.y(), d.z());
                            double H11 = wi1.dot(n11);
                            double H12 = wi1.dot(n12);
                            double H13 = wi1.dot(n13);
                            double H1 = H11 * bary.x() + H12 * bary.y() + H13 * bary.z();

                            Ray3f ray1(vertex1, wi1.normalized());
                            if (H1 > 0.0 && !scene->rayIntersect(ray1)) {
                                return H1;
                            }
                            return 0.0;
                        };

                        auto shCoeff1 = sh::ProjectFunction(SHOrder, ShFuncBounce, m_SampleCount);
                        double bounceH = 0.0;
